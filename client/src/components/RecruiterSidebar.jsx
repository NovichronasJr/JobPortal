...
export default function RecruiterSidebar() {
  ...
  // 2. Image Logic (Consistent with Candidate but for Recruiter)
  const imageUrl = useMemo(() => {
    // Note: Recruiter profile pic logic should match your backend storage path
    ...
    <aside className="fixed left-0 top-0 h-screen w-72 bg-[#020617]/80 backdrop-blur-2xl border-r border-slate-800/60 p-6 flex flex-col justify-between hidden lg:flex z-50">
      ...
      <div>
        ...
        <div className="flex items-center gap-4 mb-12 px-2">
          <div className="relative">
            <div className="absolute -inset-1 bg-indigo-500 rounded-xl blur opacity-20 transition duration-1000"></div>
          ...
          <div>
            <h1 className="text-xl font-black text-white tracking-tighter leading-none italic uppercase">
              Recruiter<span className="text-indigo-500">.Ops</span>
            </h1>
              ...
              <Link key={item.name} href={item.href} className="relative block group">
                ...
                <div className={`flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-300 relative z-10 ${
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className="font-bold text-sm tracking-tight uppercase tracking-widest">{item.name}</span>
                  </div>
      ...
      <div className="space-y-4">
        <div className="p-4 bg-slate-900/40 border border-slate-800/50 rounded-3xl flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-slate-950 border border-slate-800 overflow-hidden shadow-inner ring-1 ring-indigo-500/20">
             <img 
               src={imageUrl} 
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-white truncate capitalize">
              {user?.profile?.organizationName || "Recruiter"}
            </p>
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest truncate">
                {user?.id?.slice(-5).toUpperCase()} • ADMIN
              </p>
            </div>
          </div>
        </div>
