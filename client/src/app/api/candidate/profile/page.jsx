...
export default function ProfileEditor() {
  ...
  // --- FORM DATA STATE (Integrated Name Logic) ---
  const [formData, setFormData] = useState({
    firstName: "", 
    lastName: "",
    bio: "",
  ...
  // --- HYDRATE DATA FROM AUTH CONTEXT ---
  useEffect(() => {
    if (user?.profile) {
      const p = user.profile;
      setFormData((prev) => ({
  ...
  const addSkill = () => {
    if (formData.newSkill.trim() && !formData.skills.includes(formData.newSkill)) {
      setFormData(prev => ({ ...prev, skills: [...prev.skills, prev.newSkill.trim().toUpperCase()], newSkill: "" }));
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/candidate/update-profile`, {
        method: "PUT",
        credentials: "include", // Cookie-based auth sync
        body: data,
      });

      if (res.ok) {
        if (checkUser) await checkUser(); // Refresh Global User State
        setStatus("success");
        setIsEditing(false);
        setSelectedPhoto(null);
        setSelectedResume(null);
    ...
    <div className="p-4 lg:p-12 max-w-7xl mx-auto space-y-10 bg-[#020617] min-h-screen text-slate-200 selection:bg-indigo-500/30">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-slate-800/60 pb-10">
        <div>
          ...
          <p className="text-slate-500 font-medium tracking-tight italic">Managing Professional Identity & Industry Metrics.</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          {!isEditing ? (
            ...
            <div className="flex gap-2">
              <button onClick={() => setIsEditing(false)} className="bg-slate-900 border border-slate-800 text-slate-400 px-6 py-4 rounded-2xl font-bold hover:bg-slate-800 transition-all">
                Cancel
              </button>
              <button onClick={handleUpdate} disabled={isSaving} className="bg-indigo-600 hover:bg-indigo-500 text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-600/20">
                {isSaving ? <Loader2 className="animate-spin" size={18} /> : "Save Changes"}
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-slate-900/40 border-2 border-indigo-500/20 shadow-2xl shadow-indigo-500/5 p-10 rounded-[3.5rem] flex flex-col items-center text-center relative overflow-hidden group">
            ...
            <h2 className="text-3xl font-black text-white tracking-tighter leading-none italic uppercase">
               {formData.firstName} <span className="text-indigo-500">{formData.lastName}</span>
            </h2>

            <div className="mt-4 flex items-center gap-2">
              <div className="h-1.5 w-1.5 rounded-full bg-indigo-500 animate-pulse" />
              <p className="text-indigo-400 text-[10px] font-black uppercase tracking-[0.3em]">
                NODE ID: {user?.profile?._id?.slice(-5).toUpperCase() || "NEW-SYNC"}
              </p>
            </div>
          </div>

          <SectionCard title="Identity Metrics">
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="p-8 bg-slate-950/40 border border-slate-800/60 rounded-[2.5rem] space-y-6">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-1 flex items-center gap-2">
                    <Briefcase size={12} /> Industry Tenure
                  </p>
                </div>
              </div>
              {isEditing && (
                <div className="space-y-4">
                  <input 
                    type="range" min="0" max="40" value={formData.experienceYears}
                    onChange={(e) => setFormData({...formData, experienceYears: parseInt(e.target.value)})}
                    className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                  />
                  <div className="flex justify-between text-[9px] font-black text-slate-600 uppercase tracking-widest">
            ...
            <div className="space-y-6">
              {isEditing && (
                <div className="p-8 bg-indigo-500/5 border border-indigo-500/20 rounded-[2.5rem] space-y-4">
                  ...
                  <button onClick={addEducation} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-600/10">
                    Add Qualification
                  </button>
                </div>
              )}
                ...
                {formData.education.map((edu, idx) => (
                  <div key={idx} className="relative group p-6 bg-slate-950/40 border border-slate-800/60 rounded-3xl hover:border-indigo-500/30 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-indigo-500/10 rounded-xl text-indigo-400"><GraduationCap size={20} /></div>
                      <div>
                        <h5 className="text-white font-bold text-sm tracking-tight italic uppercase">{edu.degree}</h5>
                        <p className="text-indigo-400 text-[10px] font-bold uppercase tracking-widest mt-0.5">{edu.institution}</p>
                        <p className="text-slate-600 text-[10px] mt-1 font-bold italic tracking-tighter">Class of {edu.year}</p>
                      </div>
                    </div>
                    {isEditing && <X size={16} className="absolute top-4 right-4 text-slate-800 hover:text-red-500 cursor-pointer transition-colors" onClick={() => removeEducation(idx)} />}
                  </div>
                ))}
              </div>
            </div>
          ...
          <SectionCard title="Verified Technical Skills">
            <div className="bg-slate-950/60 p-8 rounded-[2rem] border border-slate-800">
              {isEditing && (
                <div className="flex gap-3 mb-8">
                  <input type="text" className="flex-1 bg-slate-900 border border-slate-700 rounded-2xl px-6 py-4 text-sm text-white outline-none focus:border-indigo-500/50" placeholder="Add skill (e.g., REACT)..." value={formData.newSkill} onChange={(e) => setFormData({...formData, newSkill: e.target.value})} onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addSkill())} />
                  <button onClick={addSkill} className="bg-indigo-600 text-white px-8 rounded-2xl hover:bg-indigo-500 transition-all"><Plus size={20} /></button>
                </div>
              )}
              <div className="flex flex-wrap gap-3">
                {formData.skills.map((skill) => (
                  <span key={skill} className="px-5 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 flex items-center gap-3">
                    {skill} {isEditing && <X size={14} className="cursor-pointer hover:text-red-400" onClick={() => removeSkill(skill)} />}
                  </span>
                ))}
              </div>
            </div>
          ...
          <SectionCard title="Professional Summary">
            <textarea className="w-full bg-slate-950/50 border border-slate-800 rounded-3xl p-8 text-sm text-slate-300 min-h-[150px] outline-none focus:border-indigo-500/30 transition-all italic" value={formData.bio} onChange={(e) => setFormData({...formData, bio: e.target.value})} readOnly={!isEditing} placeholder="Synthesizing mission objectives..." />
          </SectionCard>

          <SectionCard title="Decision Assets">
            <div className={`p-8 rounded-[2rem] border-2 border-dashed transition-all ${isEditing ? 'border-indigo-500/30' : 'border-slate-800'}`}>
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-5">
                  <FileText className={hasResume ? "text-emerald-400" : "text-slate-700"} size={32} />
                  <div>
                    <h4 className="text-white font-bold italic uppercase tracking-tight">{hasResume ? "Resume Synced" : "No Resume Detected"}</h4>
                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Marketplace Standard PDF</p>
                  </div>
                </div>
                {isEditing ? (
                  <button onClick={() => resumeInputRef.current.click()} className="text-[10px] font-black uppercase tracking-widest text-indigo-500 hover:text-indigo-400 transition-colors">Update Node</button>
                ) : (
                  hasResume && <a href={`${BACKEND_URL}/${user?.profile?.resumeUrl}`} target="_blank" className="text-slate-400 border border-slate-800 px-6 py-3 rounded-xl font-bold text-[10px] uppercase hover:bg-white hover:text-black transition-all">Preview</a>
                )}
                <input type="file" ref={resumeInputRef} hidden onChange={(e) => setSelectedResume(e.target.files[0])} accept=".pdf" />
              </div>
            </div>
...
function InputField({ label, icon, value, onChange, readOnly = false }) {
  return (
    <div className="space-y-4 w-full">
      <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest ml-1">{label}</label>
      <div className={`flex items-center gap-5 bg-slate-950 border ${readOnly ? 'border-slate-800/40' : 'border-indigo-500/30'} rounded-2xl px-6 py-4 shadow-inner transition-all`}>
        <span className="text-slate-700">{icon}</span>
        <input className="bg-transparent text-sm text-slate-200 outline-none w-full font-medium" value={value} onChange={onChange} disabled={readOnly} />
      </div>
    </div>
  );
}