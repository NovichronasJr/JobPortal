...
import { 
  ...
  Target, Zap, X, Code2, Loader2, Clock, ShieldCheck,
} from "lucide-react";
      ...
      <div className="space-y-6">
        {data.applications.map((app, idx) => {
          ...
          return (
            ...
            >
              <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 cursor-pointer" onClick={() => toggleDropdown(app._id)}>
                <div className="flex items-center gap-6">
                  <div className="relative">
                    <div className={`w-16 h-16 rounded-2xl bg-slate-800 overflow-hidden border-2 transition-all ${hasAdvanced ? 'border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'border-slate-700'}`}>
                      <img 
                    ...
                    {hasAdvanced && <div className="absolute -top-2 -right-2 w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center text-black border-4 border-[#020617]"><Zap size={10} fill="black" /></div>}
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-white uppercase italic tracking-tight mb-1">{c.firstName} {c.lastName}</h3>
                    <div className="flex items-center gap-4">
                      <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">{c.experienceYears} Years Exp</span>
                      {hasAdvanced && (
                        <span className={`text-[9px] font-black uppercase tracking-[0.2em] italic flex items-center gap-1 ${isInterviewing ? 'text-amber-400 animate-pulse' : 'text-emerald-400'}`}>
                ...
                <div className="flex items-center gap-10">
                  <div className="text-right">
                    ...
                    <div className={`text-2xl font-black italic flex items-center gap-2 ${hasAdvanced ? 'text-emerald-400' : 'text-slate-200'}`}>{app.aiScore}% <Sparkles size={16} className="text-indigo-500" /></div>
                  </div>
                  <div className={`w-12 h-12 rounded-2xl bg-slate-900 flex items-center justify-center text-slate-500 transition-transform duration-500 ${isExpanded ? 'rotate-180 text-indigo-400' : ''}`}><ChevronDown size={20} /></div>
                </div>
              </div>

              <AnimatePresence>
                {isExpanded && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-white/5 bg-slate-900/20">
                    <div className="p-8 lg:p-12">
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
                        <div className="space-y-8">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">{c.skills?.map(skill => (<span key={skill} className="px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-[9px] font-black text-slate-400 uppercase tracking-widest">{skill}</span>))}</div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-8">
                          <div>
                            ...
                            <div className="space-y-4">{c.education?.map((edu, i) => (<div key={i} className="flex justify-between items-start"><div><p className="text-white font-bold text-sm uppercase italic">{edu.degree}</p><p className="text-[10px] font-bold text-indigo-500 uppercase">{edu.institution}</p></div><span className="text-[9px] font-black text-slate-700">{edu.year}</span></div>))}</div>
                          </div>

                          {/* --- FIXED: RESUME CHECK GUARD --- */}
                          <button 
                            disabled={!c.resumeUrl}
                            onClick={() => setViewingResume(c.resumeUrl)}
                            className={`w-full py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 transition-all group ${
                                c.resumeUrl 
                                ? "bg-slate-950 border border-slate-800 text-slate-400 hover:bg-white hover:text-black" 
                                : "bg-slate-900/50 border border-slate-800/50 text-slate-600 cursor-not-allowed opacity-50"
                            }`}
                          >
                            {c.resumeUrl ? "Launch Neural Asset Viewer" : "No Resume Node Found"}
                            {c.resumeUrl ? <FileText size={18} className="group-hover:animate-bounce" /> : <X size={16} />}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-3">
                           ...
                           <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Current Status: <span className={hasAdvanced ? 'text-emerald-400' : 'text-slate-300'}>{app.status}</span></div>
                        </div>
                        {app.status === "Applied" ? (
                          <button disabled={updatingId === app._id} onClick={() => handleShortlist(app._id)} className="w-full md:w-auto px-12 py-5 bg-indigo-600 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-[0.3em] flex items-center justify-center gap-3 hover:bg-indigo-500 transition-all shadow-xl shadow-indigo-600/20 active:scale-95 disabled:opacity-50">
                            {updatingId === app._id ? <Loader2 className="animate-spin" size={16}/> : <Zap size={16} fill="white"/>} Establish Sync
                          </button>
                        ) : (
                          <div className={`px-10 py-5 border rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center gap-3 italic ${isInterviewing ? 'bg-amber-500/10 border-amber-500/30 text-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.1)]' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'}`}>{isInterviewing ? <Clock size={16} /> : <Sparkles size={16} />}{app.status === "Shortlisted" ? "Candidate Shortlisted" : `${app.status} phase active`}</div>
                        )}
                      </div>
                    </div>
                  </motion.div>
      ...
      <AnimatePresence>
        {viewingResume && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed inset-y-0 right-0 w-full lg:w-[850px] bg-[#020617] border-l border-white/10 z-[70] flex flex-col">
              <div className="p-8 border-b border-white/5 flex items-center justify-between">
                <div><h3 className="text-2xl font-black text-white italic uppercase tracking-tighter">Neural.Asset</h3></div>
